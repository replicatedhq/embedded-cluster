...
case "$ROLE" in
    ...
    install) APP_UPDATE_CURSOR=500 ;;
    legacydr) APP_UPDATE_CURSOR=550 ;;
    noop) APP_UPDATE_CURSOR=600 ;;
    app-only-upgrade) APP_UPDATE_CURSOR=650 ;;
    upgrade) APP_UPDATE_CURSOR=700 ;;
    *) echo "unknown bundle role: $ROLE" >&2; exit 1 ;;
esac
...
if [ -n "${EC_CONFIG_SOURCE:-}" ]; then
    cp "$EC_CONFIG_SOURCE" "$prepared/cluster-config.yaml"
fi
...
cp "$prepared/cluster-config.yaml" "$workdir/release/cluster-config.yaml"
{
    ...
    echo "channelSequence: $APP_UPDATE_CURSOR"
...
RUNTIME_ASSETS_DIR="$workdir/runtime" RUNTIME_PLAN="$workdir/runtime-plan.json" \
    ...
    PROXY_REGISTRY_DOMAIN="$(./output/bin/airgap-bundle config-proxy-domain --config "$prepared/cluster-config.yaml")" \
...
for variant in standard snapshot; do
    ...
    APPLICATION_AIRGAP_BUNDLE="$workdir/baseline/$APP_SLUG.airgap" \
        ...
        APP_UPDATE_CURSOR="$APP_UPDATE_CURSOR" \
        EC_CONFIG="$workdir/release/cluster-config.yaml" \
        EC_ARTIFACTS_DIR="$workdir/artifacts" \
        EC_REGISTRY_DIR="$registry_dir" EC_REGISTRY_IMAGE="$registry_image" \
        OUTPUT_BUNDLE="$OUTPUT_DIR/$variant/$BUNDLE_NAME.tgz" \
