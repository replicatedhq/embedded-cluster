...
function k0sbin() {
    ...
    if [ -n "${k0s_override}" ] && [ "${k0s_override}" != '' ]; then
        ...
        curl --fail-with-body --retry 5 --retry-all-errors -L -o "${k0s_version}" "${k0s_override}"
    else
        ...
        curl --fail-with-body --retry 5 --retry-all-errors -L -o "${k0s_version}" "https://github.com/k0sproject/k0s/releases/download/${k0s_version}/k0s-${k0s_version}-amd64"
    fi
...
function kotsbin() {
    ...
    if [ -n "${kots_override}" ] && [ "${kots_override}" != '' ]; then
        ...
        curl --fail-with-body --retry 5 --retry-all-errors -L -o "kots_linux_amd64.tar.gz" "${kots_override}"
    else
        ...
        curl --fail-with-body --retry 5 --retry-all-errors -L -o "kots_linux_amd64.tar.gz" "https://github.com/replicatedhq/kots/releases/download/${kots_version}/kots_linux_amd64.tar.gz"
    fi
