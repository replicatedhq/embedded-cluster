...
# A cache miss invokes this script again to materialize the already-resolved
# plan. Do not regenerate it in the child: doing so can drop images discovered
# from configured extension charts and change the digest being published.
if [ "${1:-}" = _build ]; then
    chart_args=()
...
if [ -n "${chart_dir:-}" ]; then
    # Version metadata lists extension charts but list-images does not render
    # them. Include every image produced by each chart's configured values.
    chart_configs_file="$chart_dir/chart-configs.jsonl"
    jq -c '.Configs.charts[]' "$metadata_file" > "$chart_configs_file"
    while IFS= read -r chart_config; do
        [ -z "$chart_config" ] && continue
        release_name=$(jq -er .name <<<"$chart_config")
        chart_ref=$(jq -er .chartname <<<"$chart_config")
        chart_name=${chart_ref##*/}
        chart_version=$(jq -er .version <<<"$chart_config")
        chart_path="$chart_dir/$chart_name-$chart_version.tgz"
        if [ ! -f "$chart_path" ]; then
            echo "downloaded chart not found: $chart_path" >&2
            exit 1
        fi
        values_file="$chart_dir/$chart_name-$chart_version-values.yaml"
        images_file="$chart_dir/$chart_name-$chart_version-images.txt"
        jq -r '.values // ""' <<<"$chart_config" > "$values_file"
        ./output/bin/airgap-bundle chart-images --name "$release_name" --chart "$chart_path" \
            --values-file "$values_file" > "$images_file"
        while IFS= read -r image; do
            [ -z "$image" ] || requested_images+=("$image")
        done < "$images_file"
    done < "$chart_configs_file"
fi
sorted_images_file=$(mktemp "${TMPDIR:-/tmp}/ec-runtime-images.XXXXXX")
printf '%s\n' "${requested_images[@]}" | sed '/^[[:space:]]*$/d' | sort -u > "$sorted_images_file"
mapfile -t requested_images < "$sorted_images_file"
rm -f "$sorted_images_file"
resolve_args=()
for image in "${requested_images[@]}"; do resolve_args+=(--image "$image"); done
[ -z "${PROXY_REGISTRY_DOMAIN:-}" ] || resolve_args+=(--proxy-registry-domain "$PROXY_REGISTRY_DOMAIN")
