...
function embeddedcluster() {
    ...
    # check if a file 'build/embedded-cluster-linux-amd64-previous-k0s.tgz' exists in the directory
    # if it does, upload it as releases/v${EC_VERSION}.tgz
    if [ -f build/embedded-cluster-linux-amd64-previous-k0s.tgz ]; then
        # append a 'v' prefix to the version if it doesn't already have one
        retry 3 aws s3 cp --no-progress build/embedded-cluster-linux-amd64-previous-k0s.tgz "s3://${S3_BUCKET}/releases/v${EC_VERSION#v}.tgz"
    else
        echo "build/embedded-cluster-linux-amd64-previous-k0s.tgz not found, skipping upload"
    fi
}
