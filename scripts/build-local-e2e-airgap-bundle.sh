...
augment_args=(
    ...
    --update-cursor "$APP_UPDATE_CURSOR"
    --output "$workdir/$APP_SLUG.airgap"
)
if [ -n "${EC_REGISTRY_IMAGE:-}" ]; then
    augment_args+=(--ec-registry-image "$EC_REGISTRY_IMAGE")
fi
./output/bin/airgap-bundle augment "${augment_args[@]}"
