...
deploy_app() {
    ...
    kubectl kots set config embedded-cluster-smoke-test-staging-app -n kotsadm --key="hostname" --value="123"
    kubectl kots set config embedded-cluster-smoke-test-staging-app -n kotsadm --key="pw" --value="mypassword" --deploy
    echo "app versions"
    kubectl kots get versions -n kotsadm embedded-cluster-smoke-test-staging-app
