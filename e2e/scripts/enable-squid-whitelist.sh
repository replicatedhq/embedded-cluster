...
function validate_whitelist() {
    ...
    status_code=$(curl -s -i -o /dev/null -w "%{http_code}" -x http://10.0.0.254:3128 https://ec-e2e-replicated-app.testcluster.net/market/v1/echo/ip)
    ...
    status_code=$(curl -s -i -o /dev/null -w "%{http_code}" -x http://10.0.0.254:3128 https://google.com)
...
function main() {
    ...
    sed -i 's/# http_access deny all/http_access deny all/' /etc/squid/conf.d/ec.conf
