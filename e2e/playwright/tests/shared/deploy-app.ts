...
export const deployApp = async (page: Page, expect: Expect) => {
  ...
  await page.getByRole('button', { name: 'Continue' }).click();
  await expect(page.getByText('Validate the environment')).toBeVisible({ timeout: 30 * 1000 });
  await expect(page.getByRole('button', { name: 'Rerun' })).toBeVisible({ timeout: 30 * 1000 });
