...
async function verifyUpgradeSuccess(page: Page) {
  ...
  await expect(page.locator('.VersionHistoryRow').filter({ hasText: process.env.APP_UPGRADE_VERSION }).filter({ hasText: 'Currently deployed version' })).toBeVisible({ timeout: 180 * 1000 });
