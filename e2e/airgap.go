...
func downloadAirgapBundle(t *testing.T, versionLabel string, destPath string, licenseID string) error {
	if source, ok, err := localAirgapBundle(versionLabel, destPath, licenseID); ok {
		if err != nil {
			return err
		}
...
func localAirgapBundle(versionLabel, destPath, licenseID string) (string, bool, error) {
	...
	bundleName := localAirgapBundleName(versionLabel, destPath)
	path := fmt.Sprintf("%s/%s/%s.tgz", root, variant, bundleName)
...
func localAirgapBundleName(versionLabel, destPath string) string {
	if destPath == AirgapUpgradeBundlePath && versionLabel == fmt.Sprintf("appver-%s", os.Getenv("SHORT_SHA")) {
		return versionLabel + "-app-only-upgrade"
	}
	return versionLabel
}
