...
`, hostname, password)
	...
	_, _, err = tc.RunCommandOnNode(0, []string{"sh", "-c", "'" + configValuesFileB64 + "' | base64 -d > /assets/config-values.yaml"})
	if err != nil {
...
`, hostname, password)
	...
	stdout, stderr, err := tc.RunCommandOnNode(0, []string{"sh", "-c", "'" + configValuesFileB64 + "' | base64 -d > /assets/config-values.yaml"})
	if err != nil {
		t.Fatalf("fail to create config values file: %v", err)
	} else {
