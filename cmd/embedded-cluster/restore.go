...
func (e *invalidBackupsError) Error() string {
	...
	return fmt.Sprintf("Found %d backups, but none are restorable:\n%s", len(e.invalidBackups), strings.Join(reasons, "\n"))
}
...
func waitForAdditionalNodes(ctx context.Context) error {
	...
	if err := adminconsole.WaitForReady(ctx, kcli, defaults.KotsadmNamespace); err != nil {
