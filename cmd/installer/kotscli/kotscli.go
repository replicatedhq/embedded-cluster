...
func createConfigValuesFile(configValues *kotsv1beta1.ConfigValues, rc runtimeconfig.RuntimeConfig) (string, error) {
	...
	// Set TypeMeta fields for proper YAML serialization
	configValues.APIVersion = "kots.io/v1beta1"
	configValues.Kind = "ConfigValues"

	// Set a meaningful name to reflect these were generated by embedded cluster
	configValues.ObjectMeta = metav1.ObjectMeta{
		Name: "kots-app-config",
	}

	// Use Kubernetes-specific YAML serialization to properly handle TypeMeta and ObjectMeta
	data, err := k8syaml.Marshal(configValues)
	if err != nil {
