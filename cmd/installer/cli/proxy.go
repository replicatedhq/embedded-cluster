...
func addProxyFlags(cmd *cobra.Command, consoleConfig *console.Config) error {
	cmd.Flags().StringVar(&consoleConfig.HTTPProxy, "http-proxy", "", "HTTP proxy to use for the installation (overrides http_proxy/HTTP_PROXY environment variables)")
	cmd.Flags().StringVar(&consoleConfig.HTTPSProxy, "https-proxy", "", "HTTPS proxy to use for the installation (overrides https_proxy/HTTPS_PROXY environment variables)")
	cmd.Flags().StringVar(&consoleConfig.NoProxy, "no-proxy", "", "Comma-separated list of hosts for which not to use a proxy (overrides no_proxy/NO_PROXY environment variables)")
