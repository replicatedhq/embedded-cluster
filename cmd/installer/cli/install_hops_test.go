	...
	tests := []struct {
	...
	}{
		{
			...
			validate: func(t *testing.T, opts apiOptions) {
				req := require.New(t)
				req.Equal(apitypes.InstallTargetLinux, opts.APIConfig.InstallTarget)
				req.Equal("password123", opts.APIConfig.Password)
				req.NotEmpty(opts.APIConfig.PasswordHash)
				err := bcrypt.CompareHashAndPassword(opts.APIConfig.PasswordHash, []byte("password123"))
				req.NoError(err)
				req.Equal([]byte("cert-data"), opts.APIConfig.TLSConfig.CertBytes)
				req.Equal([]byte("key-data"), opts.APIConfig.TLSConfig.KeyBytes)
				req.Equal("example.com", opts.APIConfig.TLSConfig.Hostname)
				req.Equal([]byte("license-data"), opts.APIConfig.License)
				req.Equal("/path/to/bundle.airgap", opts.APIConfig.AirgapBundle)
				req.NotNil(opts.APIConfig.AirgapMetadata)
				req.Equal(int64(1024*1024), opts.APIConfig.EmbeddedAssetsSize)
				req.Equal(apitypes.AppConfigValues{"key1": {Value: "value1"}, "key2": {Value: "value2"}}, opts.APIConfig.ConfigValues)
				req.NotNil(opts.APIConfig.EndUserConfig)
				req.Equal("cluster-123", opts.APIConfig.ClusterID)
				req.Equal(apitypes.ModeInstall, opts.APIConfig.Mode)
				req.Equal(30303, opts.APIConfig.LinuxConfig.RuntimeConfig.AdminConsolePort())
				req.Equal(30304, opts.APIConfig.KubernetesConfig.Installation.AdminConsolePort())
				req.Equal(false, opts.APIConfig.RequiresInfraUpgrade)
				req.Equal(8800, opts.ManagerPort)
				req.Equal(false, opts.Headless)
				req.Equal(true, opts.APIConfig.LinuxConfig.AllowIgnoreHostPreflights)
		...
		},
		{
			...
			validate: func(t *testing.T, opts apiOptions) {
				req := require.New(t)
				req.Equal(apitypes.InstallTargetKubernetes, opts.APIConfig.InstallTarget)
				req.Equal("pass", opts.APIConfig.Password)
				req.NotEmpty(opts.APIConfig.PasswordHash)
				req.Equal("", opts.APIConfig.TLSConfig.Hostname)
				req.Equal(30000, opts.ManagerPort)
				req.Equal(true, opts.Headless)
				req.Equal(false, opts.APIConfig.LinuxConfig.AllowIgnoreHostPreflights)
			},
		},
	}
