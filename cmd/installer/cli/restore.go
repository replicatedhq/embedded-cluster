...
func RestoreCmd(ctx context.Context, name string) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if err := runRestore(cmd.Context(), name, installConfig, cliFlags, s3Store, skipStoreValidation); err != nil {
				return err
			}
	...
	if err := addRestoreInstallConfigFlags(cmd, &installConfig); err != nil {
		panic(err)
	}
	if err := addRestoreCmdFlags(cmd, &cliFlags); err != nil {
		panic(err)
	}
...
func addRestoreCmdFlags(cmd *cobra.Command, cliFlags *installCmdFlags) error {
...
func doRestore(ctx context.Context, name string, installConfig models.InstallationConfig, cliFlags installCmdFlags, s3Store s3BackupStore, skipStoreValidation bool) error {
	...
	if state != ecRestoreStateNew {
		...
		var err error
		backupToRestore, err = getBackupFromRestoreState(ctx, isAirgap)
		if err != nil {
		...
		if backupToRestore != nil {
			...
			if err := overrideRuntimeConfigFromBackup(installConfig.LocalArtifactMirrorPort, *backupToRestore); err != nil {
	...
	case ecRestoreStateNew:
		err = runRestoreStepNew(ctx, name, installConfig, cliFlags, &s3Store, skipStoreValidation)
		if err != nil {
			return err
		}
	...
	case ecRestoreStateConfirmBackup:
		...
		backup, ok, err := runRestoreStepConfirmBackup(ctx, isAirgap)
		if err != nil {
			return err
		} else if !ok {
			return nil
	...
	case ecRestoreStateWaitForNodes:
		...
		err = runRestoreWaitForNodes(ctx, backupToRestore, installConfig.NetworkInterface)
		if err != nil {
			return err
		}
	...
	case ecRestoreStateRestoreSeaweedFS:
		...
		err = runRestoreSeaweedFS(ctx, backupToRestore, isAirgap)
		if err != nil {
			return err
		}
	...
	case ecRestoreStateRestoreRegistry:
		...
		err = runRestoreRegistry(ctx, backupToRestore, isAirgap)
		if err != nil {
			return err
		}
	...
	case ecRestoreStateAdminConsoleEnableHA:
		...
		err = runRestoreEnableAdminConsoleHA(ctx, backupToRestore, installConfig, isAirgap)
		if err != nil {
			return err
		}
	...
	case ecRestoreStateRestoreExtensions:
		...
		err = runRestoreExtensions(ctx, isAirgap)
		if err != nil {
			return err
		}
...
func runRestoreStepNew(ctx context.Context, name string, installConfig models.InstallationConfig, cliFlags installCmdFlags, s3Store *s3BackupStore, skipStoreValidation bool) error {
	...
	_, err = installAndStartCluster(ctx, installConfig, cliFlags.airgapBundle, nil)
	if err != nil {
		return err
	}
...
func runRestoreStepConfirmBackup(ctx context.Context, isAirgap bool) (*disasterrecovery.ReplicatedBackup, bool, error) {
	kcli, err := kubeutils.KubeClient()
	if err != nil {
	...
	backups, err := waitForBackups(ctx, os.Stdout, kcli, k0sCfg, isAirgap)
	if err != nil {
...
func runRestoreWaitForNodes(ctx context.Context, backupToRestore *disasterrecovery.ReplicatedBackup, networkInterface string) error {
	...
	if err := waitForAdditionalNodes(ctx, highAvailability, networkInterface); err != nil {
		return err
	}
...
func runRestoreEnableAdminConsoleHA(ctx context.Context, backupToRestore *disasterrecovery.ReplicatedBackup, installConfig models.InstallationConfig, isAirgap bool) error {
	...
	err = addons.EnableAdminConsoleHA(ctx, kcli, hcli, isAirgap, installConfig.ServiceCIDR, runtimeconfig.ProxySpec(), in.Spec.Config, in.Spec.LicenseInfo)
	if err != nil {
		return err
	}
...
func runRestoreSeaweedFS(ctx context.Context, backupToRestore *disasterrecovery.ReplicatedBackup, isAirgap bool) error {
	highAvailability, err := isHighAvailabilityReplicatedBackup(*backupToRestore)
	if err != nil {
		return err
	} else if !isAirgap || !highAvailability {
...
func runRestoreRegistry(ctx context.Context, backupToRestore *disasterrecovery.ReplicatedBackup, isAirgap bool) error {
...
func runRestoreExtensions(ctx context.Context, isAirgap bool) error {
	airgapChartsPath := ""
	if isAirgap {
		airgapChartsPath = runtimeconfig.EmbeddedClusterChartsSubDir()
	}
