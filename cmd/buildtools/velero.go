...
func findVeleroVersionFromChart(ctx context.Context, hcli helm.Client, chartURL string, chartVersion string) (string, error) {
...
func findBestAWSPluginVersion(ctx context.Context, veleroVersion string) (string, error) {
	...
	if !ok {
		return "", fmt.Errorf("no aws plugin compatibility constraints found for velero version %s (see the \"veleroPluginForAWSCompatibility\" map in cmd/buildtools/velero.go to update compatibility)", veleroVersion)
	}
