...
import (
	...
	ecHelm "github.com/replicatedhq/embedded-cluster/pkg/helm"
...
func run(args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: airgap-bundle <application-identity|config-proxy-domain|separate-config|extract-release|pack-directory|chart-images|resolve-images|runtime-plan|build-runtime|complete-runtime|verify-runtime|augment|wrap-release>")
	}
	switch args[0] {
	...
	case "config-proxy-domain":
		fs := flag.NewFlagSet(args[0], flag.ContinueOnError)
		config := fs.String("config", "", "Embedded Cluster Config path")
		if err := fs.Parse(args[1:]); err != nil {
			return err
		}
		domain, err := airgapbundle.ConfigProxyRegistryDomain(*config)
		if err != nil {
			return err
		}
		fmt.Println(domain)
	...
	case "chart-images":
		fs := flag.NewFlagSet(args[0], flag.ContinueOnError)
		name := fs.String("name", "", "Helm release name used to render the chart")
		chart := fs.String("chart", "", "packaged chart path")
		valuesFile := fs.String("values-file", "", "chart values YAML path")
		if err := fs.Parse(args[1:]); err != nil {
			return err
		}
		...
		if *valuesFile != "" {
			...
			if len(data) > 0 {
				if err := yaml.Unmarshal(data, &values); err != nil {
					return fmt.Errorf("decode chart values: %w", err)
				}
			}
		}
		hcli, err := ecHelm.NewClient(ecHelm.HelmOptions{HelmPath: "helm"})
		if err != nil {
			return err
		}
		defer hcli.Close()
		images, err := ecHelm.ExtractImagesFromChartWithReleaseName(hcli, *name, *chart, "", values)
		if err != nil {
			return err
		}
	...
	case "resolve-images":
		fs := flag.NewFlagSet(args[0], flag.ContinueOnError)
		refs := stringList{}
		proxyRegistryDomain := fs.String("proxy-registry-domain", "", "custom proxy registry domain used by the cluster")
		fs.Var(&refs, "image", "image reference (repeatable)")
		if err := fs.Parse(args[1:]); err != nil {
			return err
		}
		resolved, err := airgapbundle.ResolveImages(context.Background(), airgapbundle.RewriteProxyRegistry(refs, *proxyRegistryDomain))
	...
	case "augment":
		...
		ecConfig := fs.String("ec-config", "", "release-specific Embedded Cluster Config")
		versionLabel := fs.String("version-label", "", "final application version label")
		updateCursor := fs.String("update-cursor", "", "final application update cursor")
		output := fs.String("output", "", "output bundle")
		var registryImages stringList
		fs.Var(&registryImages, "ec-registry-image", "embedded cluster image saved in the registry directory (repeatable)")
		if err := fs.Parse(args[1:]); err != nil {
			return err
		}
		return airgapbundle.Augment(*app, *ecDir, *ecConfig, *versionLabel, *updateCursor, *output, registryImages)
	case "wrap-release":
		fs := flag.NewFlagSet(args[0], flag.ContinueOnError)
