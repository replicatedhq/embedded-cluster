...
func ExtractReleaseDataFromBinary(exe string) ([]byte, error) {
	...
	fmt.Println("++++ endd", end)
	fmt.Println("++++ start+len(beginReleaseDelimiterBytes())", start+len(beginReleaseDelimiterBytes()))
	fmt.Println("++++ len(binContent)", len(binContent))
