...
func (a *Applier) GenerateHelmConfigs(additionalCharts []v1beta1.Chart, additionalRepositories []v1beta1.Repository) ([]v1beta1.Chart, []v1beta1.Repository, error) {
	...
	var charts []v1beta1.Chart
	var repositories []v1beta1.Repository
	for _, addon := range addons {
		...
		values, err := a.endUserConfig.Spec.ApplyEndUserAddOnOverrides(addon.Name(), addonChartConfig.Values)
		if err != nil {
			return nil, nil, fmt.Errorf("unable to apply user overrides for %s: %w", addon.Name(), err)
		}
		addonChartConfig.Values = values
		charts = append(charts, *addonChartConfig)
	}
...
func (a *Applier) GetAirgapCharts() ([]v1beta1.Chart, []v1beta1.Repository, error) {
	reg, err := registry.New(registry.Options{IsAirgap: true})
	if err != nil {
	...
	var charts []v1beta1.Chart
	if regChart != nil {
		charts = append(charts, *regChart)
	}
	var repositories []v1beta1.Repository
	if regRepo != nil {
		repositories = append(repositories, *regRepo)
	}
	return charts, repositories, nil
}
...
func (a *Applier) GetBuiltinCharts() (map[string]k0sconfig.HelmExtensions, error) {
	...
	vel, err := velero.New(velero.Options{IsEnabled: true})
	if err != nil {
	...
	var charts []v1beta1.Chart
	if velChart != nil {
		charts = append(charts, *velChart)
	}
	var repositories []v1beta1.Repository
	if velRepo != nil {
		repositories = append(repositories, *velRepo)
	}
...
func (a *Applier) load() ([]AddOn, error) {
	...
	embedoperator, err := embeddedclusteroperator.New(
		embeddedclusteroperator.Options{
