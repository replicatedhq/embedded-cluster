...
func (a *AdminConsole) ensureCAConfigmap(ctx context.Context, opts types.InstallOptions) error {
	...
	if opts.IsDryRun {
		checksum, err := calculateFileChecksum(opts.HostCABundlePath)
		if err != nil {
			return fmt.Errorf("calculate checksum: %w", err)
		}
		new, err := newCAConfigMap(opts.HostCABundlePath, checksum)
		if err != nil {
	...
	err := EnsureCAConfigmap(ctx, a.logf, a.kcli, a.mcli, opts.HostCABundlePath)
