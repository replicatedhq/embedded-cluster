...
func (m *MockK0s) Install(rc runtimeconfig.RuntimeConfig, networkInterface string) error {
	args := m.Called(rc, networkInterface)
	return args.Error(0)
}
...
func (m *MockK0s) WriteK0sConfig(ctx context.Context, networkInterface string, airgapBundle string, podCIDR string, serviceCIDR string, overrides string, mutate func(*k0sv1beta1.ClusterConfig) error) (*k0sv1beta1.ClusterConfig, error) {
	args := m.Called(ctx, networkInterface, airgapBundle, podCIDR, serviceCIDR, overrides, mutate)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
