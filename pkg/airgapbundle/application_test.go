...
func TestConfigProxyRegistryDomain(t *testing.T) {
	path := filepath.Join(t.TempDir(), "config.yaml")
	mustWrite(t, path, "apiVersion: embeddedcluster.replicated.com/v1beta1\nkind: Config\nspec:\n  domains:\n    proxyRegistryDomain: proxy.example.test\n")
	domain, err := ConfigProxyRegistryDomain(path)
	if err != nil {
		t.Fatal(err)
	}
	if domain != "proxy.example.test" {
		t.Fatalf("unexpected proxy registry domain %q", domain)
	}
}

func TestRewriteProxyRegistry(t *testing.T) {
	got := RewriteProxyRegistry([]string{
		"proxy.replicated.com/library/one:1",
		"proxy.staging.replicated.com/library/two:2",
		"registry.example.com/team/three:3",
	}, "proxy.example.test")
	want := []string{
		"proxy.example.test/library/one:1",
		"proxy.example.test/library/two:2",
		"registry.example.com/team/three:3",
	}
	for i := range want {
		if got[i] != want[i] {
			t.Fatalf("image %d: got %q, want %q", i, got[i], want[i])
		}
	}
}

func TestRuntimeArchiveImageName(t *testing.T) {
	tests := map[string]string{
		"registry.example.com/team/image:1.2.3@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa":      "registry.example.com/team/image@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
		"registry.example.com:5000/team/image:1.2.3@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa": "registry.example.com:5000/team/image@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
		"registry.example.com/team/image:1.2.3": "registry.example.com/team/image:1.2.3",
		"registry.example.com/team/image@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa": "registry.example.com/team/image@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
	}
	for input, want := range tests {
		if got := RuntimeArchiveImageName(input); got != want {
			t.Errorf("RuntimeArchiveImageName(%q) = %q, want %q", input, got, want)
		}
	}
}
...
func TestAugmentUsesProductionV2Layout(t *testing.T) {
	...
	appReleaseDir := filepath.Join(root, "app-release")
	mustWrite(t, filepath.Join(appReleaseDir, "application.yaml"), "kind: Application\n")
	if err := os.MkdirAll(appDir, 0o755); err != nil {
		t.Fatal(err)
	}
	if err := WriteDeterministicTarGz(appReleaseDir, filepath.Join(appDir, "app.tar.gz")); err != nil {
		t.Fatal(err)
	}
	...
	ecConfig := filepath.Join(root, "cluster-config.yaml")
	mustWrite(t, ecConfig, `apiVersion: embeddedcluster.replicated.com/v1beta1
kind: Config
spec:
  roles:
    controller:
...
`)
	registryImage := "proxy.example.com/anonymous/replicated/embedded-cluster-local-artifact-mirror:v1"
	if err := Augment(appBundle, ec, ecConfig, "final-version", "700", output, []string{registryImage}); err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(string(metadataData), `updateCursor: "700"`) {
		t.Fatalf("airgap metadata does not contain final update cursor:\n%s", metadataData)
	}
	if !strings.Contains(string(metadataData), "- "+registryImage) {
		t.Fatalf("airgap metadata does not contain embedded cluster registry image:\n%s", metadataData)
	}
	applicationRelease := filepath.Join(root, "extracted-application")
	if err := extractTarGz(filepath.Join(extracted, "app.tar.gz"), applicationRelease); err != nil {
		t.Fatal(err)
	}
	for _, want := range []string{"application.yaml", "cluster-config.yaml"} {
		if _, err := os.Stat(filepath.Join(applicationRelease, want)); err != nil {
			t.Errorf("missing %s from nested application release", want)
		}
	}
	normalizedConfig, err := os.ReadFile(filepath.Join(applicationRelease, "cluster-config.yaml"))
	if err != nil {
		t.Fatal(err)
	}
	for _, want := range []string{
		...
		"kots.io/embedded-cluster-role-0: controller-test",
	} {
		if !strings.Contains(string(normalizedConfig), want) {
			t.Errorf("normalized Config does not contain %q:\n%s", want, normalizedConfig)
		}
	}
	names := archiveNames(t, output)
	for _, want := range []string{"app.tar.gz", "airgap.yaml", "embedded-cluster/embedded-cluster-amd64", "embedded-cluster/artifacts/kots.tar.gz", "embedded-cluster/registry/docker/registry/v2/repositories/x"} {
		if !names[want] {
