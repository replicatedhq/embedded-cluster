...
func Augment(applicationBundle, ecDir, ecConfig, versionLabel, updateCursor, output string, registryImages []string) error {
	if versionLabel == "" {
		return fmt.Errorf("version label is required")
	}
	if updateCursor == "" {
		return fmt.Errorf("update cursor is required")
	}
	...
	if err := injectECConfig(filepath.Join(tmp, "app.tar.gz"), ecConfig); err != nil {
		return err
	}
	if err := updateAirgapMetadata(tmp, destEC, versionLabel, updateCursor, registryImages); err != nil {
		return err
	}
...
func injectECConfig(applicationArchive, ecConfig string) error {
	workspace, err := os.MkdirTemp("", "ec-application-release-")
	if err != nil {
		return err
	}
	defer os.RemoveAll(workspace)
	if err := extractTarGz(applicationArchive, workspace); err != nil {
		return fmt.Errorf("extract application release: %w", err)
	}
	if err := writeNormalizedECConfig(ecConfig, filepath.Join(workspace, "cluster-config.yaml")); err != nil {
		return fmt.Errorf("add Embedded Cluster Config: %w", err)
	}
	if err := WriteDeterministicTarGz(workspace, applicationArchive); err != nil {
		return fmt.Errorf("repack application release: %w", err)
	}
	return nil
}

// writeNormalizedECConfig applies the controller role labels that the installer
// adds to Installation.spec.config. KOTS compares that stored spec with the
// Config in a later application release byte-for-byte when deciding whether an
// app update also requires a cluster upgrade.
func writeNormalizedECConfig(source, dest string) error {
	...
	if err := yaml.Unmarshal(b, &config); err != nil {
		return fmt.Errorf("decode Embedded Cluster Config: %w", err)
	}
	if config.Spec.Roles.Controller.Labels == nil {
		config.Spec.Roles.Controller.Labels = map[string]string{}
	}
	controllerRole := config.Spec.Roles.Controller.Name
	if controllerRole == "" {
		controllerRole = "controller"
	}
	config.Spec.Roles.Controller.Labels["kots.io/embedded-cluster-role"] = "total-1"
	config.Spec.Roles.Controller.Labels["kots.io/embedded-cluster-role-0"] = controllerRole
	b, err = yaml.Marshal(&config)
	if err != nil {
		return fmt.Errorf("encode Embedded Cluster Config: %w", err)
	}
	return os.WriteFile(dest, b, 0o644)
}

func updateAirgapMetadata(workspace, ecDir, versionLabel, updateCursor string, registryImages []string) error {
	...
	metadata.Spec.UpdateCursor = updateCursor
	...
	if info, err := os.Stat(filepath.Join(ecDir, "registry")); err == nil && info.IsDir() {
		...
		if len(registryImages) == 0 {
			return fmt.Errorf("embedded cluster registry images are required when a registry directory is present")
		}
		a.Registry.SavedImages = registryImages
	}
