...
func RemapHelm(provider *defaults.Provider, cfg *v1beta1.ClusterConfig) {
	...
	for idx := range cfg.Spec.Extensions.Helm.Charts {
		cfg.Spec.Extensions.Helm.Charts[idx].ChartName = helmChartHostPath(provider, cfg.Spec.Extensions.Helm.Charts[idx])
	}
}

func helmChartHostPath(provider *defaults.Provider, chart v1beta1.Chart) string {
	return filepath.Join(provider.EmbeddedClusterChartsSubDir(), fmt.Sprintf("%s-%s.tgz", chart.Name, chart.Version))
}
