...
func MaterializeAirgap(dataDir string, airgapReader io.Reader) error {
	// decompress tarball
	...
	for {
		...
		if nextFile.Name == "embedded-cluster/images-amd64.tar" {
			err = writeOneFile(tarreader, filepath.Join(paths.K0sSubDir(dataDir), K0sImagePath), nextFile.Mode)
			if err != nil {
		...
		if nextFile.Name == "embedded-cluster/charts.tar.gz" {
			err = writeChartFiles(dataDir, tarreader)
			if err != nil {
...
func FetchAndWriteArtifacts(ctx context.Context, kotsAPIAddress string, dataDir string) error {
	...
	if err := writeOneFile(imagesFile, filepath.Join(paths.K0sSubDir(dataDir), K0sImagePath), 0644); err != nil {
		return fmt.Errorf("failed to write k0s images file: %w", err)
	}
	...
	if err := writeChartFiles(dataDir, chartsTGZ); err != nil {
		return fmt.Errorf("failed to write chart files: %w", err)
	}
	return nil
}
...
func writeChartFiles(dataDir string, reader io.Reader) error {
	// decompress tarball
	...
	for {
		...
		subdir := paths.ChartsSubDir(dataDir)
