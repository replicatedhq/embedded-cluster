...
// RestartStatefulSetPods deletes and recreates StatefulSet pods in sequential order
// to ensure they pick up new configuration. This is specifically needed for SeaweedFS
// masters when scaling from single to HA mode during upgrades.
func (k *KubeUtils) RestartStatefulSetPods(ctx context.Context, cli client.Client, namespace, name string) error {
	...
	nsn := client.ObjectKey{Namespace: namespace, Name: name}
	if err := cli.Get(ctx, nsn, &sts); err != nil {
		return fmt.Errorf("get statefulset %s/%s: %w", namespace, name, err)
	}

	if sts.Spec.Replicas == nil {
		return fmt.Errorf("statefulset %s/%s has nil replicas", namespace, name)
	}

	replicas := int(*sts.Spec.Replicas)

	// Delete pods in order (0 to n-1) and wait for each to be recreated
	for i := 0; i < replicas; i++ {
		podName := fmt.Sprintf("%s-%d", name, i)

		// Delete the pod
		var pod corev1.Pod
		podNsn := client.ObjectKey{Namespace: namespace, Name: podName}
		if err := cli.Get(ctx, podNsn, &pod); err != nil {
			if k8serrors.IsNotFound(err) {
		...
		// Wait for the pod to be deleted and recreated as ready
		if err := k.waitForPodRecreatedAndReady(ctx, cli, namespace, podName); err != nil {
			return fmt.Errorf("wait for pod %s/%s to be ready: %w", namespace, podName, err)
		}
	}
...
// waitForPodRecreatedAndReady waits for a pod to be deleted and then recreated and ready
func (k *KubeUtils) waitForPodRecreatedAndReady(ctx context.Context, cli client.Client, namespace, name string) error {
	...
	if err := wait.ExponentialBackoffWithContext(
		ctx, backoff, func(ctx context.Context) (bool, error) {
			...
			if k8serrors.IsNotFound(err) {
				// Pod is still being deleted, continue waiting
				lasterr = fmt.Errorf("pod %s/%s still being deleted", namespace, name)
				return false, nil
			} else if err != nil {
				lasterr = fmt.Errorf("get pod: %w", err)
				return false, nil
			...
			lasterr = fmt.Errorf("pod %s/%s not ready yet", namespace, name)
			return false, nil
		},
	); err != nil {
		if errors.Is(err, context.Canceled) {
			...
			return fmt.Errorf("timed out waiting for pod %s/%s to be ready: %w", namespace, name, lasterr)
		} else {
