...
func Run(ctx context.Context, spec *troubleshootv1beta2.HostPreflightSpec, proxy *ecv1beta1.ProxySpec) (*apitypes.HostPreflightOutput, string, error) {
...
func pathEnv(env []string) []string {
	...
	if path != "" {
		next = append(next, fmt.Sprintf("PATH=%s:%s", path, runtimeconfig.EmbeddedClusterBinsSubDir()))
	} else {
		next = append(next, fmt.Sprintf("PATH=%s", runtimeconfig.EmbeddedClusterBinsSubDir()))
	}
	return next
}
