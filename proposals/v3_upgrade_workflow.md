...
└────────────────────────┬───────────────────────────────────┘
                         │
┌────────────────────────▼────────────────────────────────────┐
│                    KOTS CLI Integration                     │
│  - kubectl kots upstream upgrade --airgap-bundle            │
│  - kots set config --config-file                            │
└─────────────────────────────────────────────────────────────┘
```
...
StateNew
  ↓
StateAppUpgrading → StateAppUpgradeSucceeded/StateAppUpgradeFailed
```
...
cmd/installer/kotscli/upgrade.go                  # KOTS CLI upgrade implementation
...
func (m *appUpgradeManager) upgrade(ctx context.Context, configValues kotsv1beta1.ConfigValues) error {
    ...
    upgradeOpts := kotscli.UpgradeOptions{
    ...
    upgradeOpts.ConfigValuesFile = configValuesFile

    if m.kotsCLI != nil {
        return m.kotsCLI.Upgrade(upgradeOpts)
    }

    return kotscli.Upgrade(upgradeOpts)
}
```
...
KOTS CLI Upgrade Options:
```go
// cmd/installer/kotscli/upgrade.go
...
func Upgrade(opts UpgradeOptions) error {
    // Pseudo code: Execute kubectl kots upstream upgrade command
    // 
    // Command: kubectl kots upstream upgrade <app-slug>
    // Flags:
    //   --license-file <temp-license-file>
    //   --namespace <opts.Namespace>
    //   --app-version-label <version-from-release>
    //   --airgap-bundle <opts.AirgapBundle> (if provided)
    //   --config-file <opts.ConfigValuesFile> (if provided)
...
**APIs consumed:**
- KOTS CLI (`kubectl-kots upstream upgrade`) - Handles license sync automatically
...
`kots upstream upgrade` automatically handles license synchronization as part of the upgrade process. This eliminates the need for separate license management logic in our upgrade workflow. The KOTS CLI:
- Fetches the latest license from the upstream
- Validates license compatibility with the new version
- Updates the license in the cluster automatically
- Handles both airgap and online license scenarios

This automatic license sync means we don't need to implement any license-specific endpoints or UI components for the upgrade workflow.
...
1. New upgrade command and CLI entry point
2. Basic upgrade controller and manager
3. Core upgrade API endpoints (/app/upgrade, /app/upgrade/status)
4. Frontend mode detection and step filtering (skip config/preflights)
5. Upgrade execution via KOTS CLI
