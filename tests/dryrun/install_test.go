...
// this test is to verify CA bundle configuration in Helm values for addons
func TestCABundleConfiguration(t *testing.T) {
	...
	// Mocking the values that would be sent to Helm - CA bundle only, no HTTP proxy
	...
	adminConsoleValues := map[string]any{
		"extraEnv": []map[string]any{
			{
				"name":  "SSL_CERT_DIR",
				"value": "/certs",
	...
	assert.Equal(t, "/certs", operatorValues["extraEnv"].([]map[string]any)[0]["value"])
	vol := operatorValues["extraVolumes"].([]map[string]any)[0]
	assert.Equal(t, "host-ca-bundle", vol["name"])
	assert.Equal(t, hostCABundle, vol["hostPath"].(map[string]any)["path"])
	assert.Equal(t, "FileOrCreate", vol["hostPath"].(map[string]any)["type"])

	volMount := operatorValues["extraVolumeMounts"].([]map[string]any)[0]
	assert.Equal(t, "host-ca-bundle", volMount["name"])
	assert.Equal(t, "/certs/ca-certificates.crt", volMount["mountPath"])

	// Verify Velero values
	assert.Equal(t, "/certs", veleroValues["configuration.extraEnvVars"].(map[string]any)["SSL_CERT_DIR"])
	vol = veleroValues["extraVolumes"].([]map[string]any)[0]
	assert.Equal(t, "host-ca-bundle", vol["name"])
	assert.Equal(t, hostCABundle, vol["hostPath"].(map[string]any)["path"])
	assert.Equal(t, "FileOrCreate", vol["hostPath"].(map[string]any)["type"])

	volMount = veleroValues["extraVolumeMounts"].([]map[string]any)[0]
	assert.Equal(t, "host-ca-bundle", volMount["name"])
	assert.Equal(t, "/certs/ca-certificates.crt", volMount["mountPath"])

	// Verify Admin Console values
	assert.Equal(t, "/certs", adminConsoleValues["extraEnv"].([]map[string]any)[0]["value"])
	vol = adminConsoleValues["extraVolumes"].([]map[string]any)[0]
	assert.Equal(t, "host-ca-bundle", vol["name"])
	assert.Equal(t, hostCABundle, vol["hostPath"].(map[string]any)["path"])
	assert.Equal(t, "FileOrCreate", vol["hostPath"].(map[string]any)["type"])

	volMount = adminConsoleValues["extraVolumeMounts"].([]map[string]any)[0]
	assert.Equal(t, "host-ca-bundle", volMount["name"])
	assert.Equal(t, "/certs/ca-certificates.crt", volMount["mountPath"])
}

// this test is to verify HTTP proxy configuration in Helm values for addons
func TestHTTPProxyConfiguration(t *testing.T) {
	...
	// Mocking the values that would be sent to Helm
	operatorValues := map[string]any{
		"extraEnv": []map[string]any{
			{
				"name":  "HTTP_PROXY",
	...
	// Verify embedded-cluster-operator values
	assert.Equal(t, "http://localhost:3128", operatorValues["extraEnv"].([]map[string]any)[0]["value"])
	assert.Equal(t, "http://localhost:3128", operatorValues["extraEnv"].([]map[string]any)[1]["value"])
	assert.Equal(t, "localhost,127.0.0.1,10.0.0.0/8", operatorValues["extraEnv"].([]map[string]any)[2]["value"])

	// Verify Velero values
	assert.Equal(t, "http://localhost:3128", veleroValues["configuration.extraEnvVars"].(map[string]any)["HTTP_PROXY"])
	assert.Equal(t, "http://localhost:3128", veleroValues["configuration.extraEnvVars"].(map[string]any)["HTTPS_PROXY"])
	assert.Equal(t, "localhost,127.0.0.1,10.0.0.0/8", veleroValues["configuration.extraEnvVars"].(map[string]any)["NO_PROXY"])

	// Verify Admin Console values
	assert.Equal(t, "http://localhost:3128", adminConsoleValues["extraEnv"].([]map[string]any)[1]["value"])
	assert.Equal(t, "http://localhost:3128", adminConsoleValues["extraEnv"].([]map[string]any)[2]["value"])
	assert.Equal(t, "localhost,127.0.0.1,10.0.0.0/8", adminConsoleValues["extraEnv"].([]map[string]any)[3]["value"])
}
...
func TestHTTPProxyWithCABundleConfiguration(t *testing.T) {
	...
	// Mocking the values that would be sent to Helm with both HTTP proxy and CA bundle
	operatorValues := map[string]any{
		"extraEnv": []map[string]any{
			{
				"name":  "HTTP_PROXY",
	...
	assert.Equal(t, "/certs", operatorValues["extraEnv"].([]map[string]any)[3]["value"])
	vol := operatorValues["extraVolumes"].([]map[string]any)[0]
	assert.Equal(t, "host-ca-bundle", vol["name"])
	assert.Equal(t, hostCABundle, vol["hostPath"].(map[string]any)["path"])
	assert.Equal(t, "FileOrCreate", vol["hostPath"].(map[string]any)["type"])
	...
	// Similar verifications for Velero and Admin Console could be added here
	// but for brevity we'll just test that they include both proxy and CA bundle configs
	assert.Equal(t, "http://localhost:3128", veleroValues["configuration.extraEnvVars"].(map[string]any)["HTTP_PROXY"])
	assert.Equal(t, "/certs", veleroValues["configuration.extraEnvVars"].(map[string]any)["SSL_CERT_DIR"])

	assert.Equal(t, "http://localhost:3128", adminConsoleValues["extraEnv"].([]map[string]any)[1]["value"])
	assert.Equal(t, "/certs", adminConsoleValues["extraEnv"].([]map[string]any)[4]["value"])
}
