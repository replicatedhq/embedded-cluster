...
func WithInfra(infra types.KubernetesInfra) StoreOption {
...
func (s *memoryStore) Get() (types.KubernetesInfra, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()

	var infra types.KubernetesInfra
	if err := deepcopy.Copy(&infra, &s.infra); err != nil {
		return types.KubernetesInfra{}, err
	}
...
func (s *memoryStore) RegisterComponent(name string) error {
	...
	s.infra.Components = append(s.infra.Components, types.KubernetesInfraComponent{
		Name: name,
		Status: types.Status{
