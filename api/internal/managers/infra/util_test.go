...
func TestInfraManager_addLogs(t *testing.T) {
	tests := []struct {
		name      string
		component string
		format    string
	...
	}{
		{
			...
			args:      []interface{}{},
			expected:  "[k0s] starting installation",
		},
		{
			name:      "formatted message",
			component: "helm",
			format:    "installing chart %s in namespace %s",
			args:      []interface{}{"my-app", "production"},
			expected:  "[helm] installing chart my-app in namespace production",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call addLogs
			manager.addLogs(tt.component, tt.format, tt.args...)
...
func TestInfraManager_addLogs_WithError(t *testing.T) {
	// Create a mock store that returns an error
	mockStore := &MockStore{}
	mockStore.On("AddLogs", mock.AnythingOfType("string")).Return(assert.AnError)
	...
	// This should not panic even if AddLogs returns an error
	assert.NotPanics(t, func() {
		manager.addLogs("test", "error message")
	})
