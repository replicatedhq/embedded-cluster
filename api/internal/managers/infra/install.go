...
func (m *infraManager) Install(ctx context.Context, config *types.InstallationConfig) (finalErr error) {
	...
	installed, err := m.k0s.IsInstalled()
	if err != nil {
		return err
	}
	if installed {
...
func (m *infraManager) installK0s(ctx context.Context, config *types.InstallationConfig, proxy *ecv1beta1.ProxySpec) (k0sCfg *k0sv1beta1.ClusterConfig, finalErr error) {
	...
	k0sCfg, err := m.k0s.WriteK0sConfig(ctx, config.NetworkInterface, m.airgapBundle, config.PodCIDR, config.ServiceCIDR, m.endUserConfig, nil)
	if err != nil {
		return nil, fmt.Errorf("create config file: %w", err)
	}

	logFn("creating systemd unit files")
	if err := m.hostUtils.CreateSystemdUnitFiles(ctx, m.logger, m.rc, false, proxy); err != nil {
		return nil, fmt.Errorf("create systemd unit files: %w", err)
	}

	logFn("installing k0s")
	if err := m.k0s.Install(m.rc, config.NetworkInterface); err != nil {
		return nil, fmt.Errorf("install cluster: %w", err)
	}

	logFn("waiting for k0s to be ready")
	if err := m.k0s.WaitForK0s(); err != nil {
...
func (m *infraManager) installAddOns(
	...
	// progressChan is used to capture the progress of the addons
	...
	opts := m.getAddonInstallOpts(config, proxy, license)
...
func (m *infraManager) getAddonInstallOpts(config *types.InstallationConfig, proxy *ecv1beta1.ProxySpec, license *kotsv1beta1.License) addons.InstallOptions {
	ecDomains := utils.GetDomains(m.releaseData)
