...
func (m *appInstallManager) Install(ctx context.Context, installableCharts []types.InstallableHelmChart, configValues kotsv1beta1.ConfigValues, registrySettings *types.RegistrySettings) error {
	...
	// Start the namespaces informer to ensure image pull secrets and other required resources in app namespaces
	nsInformer, err := runNamespacesInformer(ctx, m.kcli, m.clientset, registrySettings, m.logger)
	if err != nil {
		return fmt.Errorf("start namespace secret sync: %w", err)
	}
	defer nsInformer.Stop()
