...
func TestAppUpgradeManager_Upgrade(t *testing.T) {
	...
	t.Run("Successful upgrade", func(t *testing.T) {
		...
		mockKotsCLI.On("Upgrade", mock.MatchedBy(func(opts kotscli.UpstreamUpgradeOptions) bool {
			return opts.AppSlug == "test-app" &&
				opts.Namespace == "kotsadm" &&
				opts.ClusterID == "test-cluster" &&
				opts.AirgapBundle == "test-airgap.tar.gz" &&
				opts.SkipPreflights == true
		})).Return(nil)

		// Mock GetVersions to return a version with matching label
		mockVersions := []kotscli.AppVersionResponse{
			{
				VersionLabel: "v1.0.0",
				Sequence:     123,
				Status:       "deployed",
		...
		mockKotsCLI.On("GetVersions", mock.MatchedBy(func(opts kotscli.GetVersionsOptions) bool {
			return opts.AppSlug == "test-app" && opts.Namespace == "kotsadm"
	...
	t.Run("Failed upgrade should set failed status", func(t *testing.T) {
		...
		// Create mock upgrader that fails
		mockKotsCLI := &kotscli.MockKotsCLI{}
		mockKotsCLI.On("Upgrade", mock.Anything).Return(assert.AnError)
	...
	t.Run("Upgrade with airgap bundle", func(t *testing.T) {
		configValues := kotsv1beta1.ConfigValues{}

		// Create mock upgrader
		mockKotsCLI := &kotscli.MockKotsCLI{}
		mockKotsCLI.On("Upgrade", mock.MatchedBy(func(opts kotscli.UpstreamUpgradeOptions) bool {
			return opts.AirgapBundle == "airgap-bundle.tar.gz"
		})).Return(nil)
		...
		mockKotsCLI.On("GetVersions", mock.Anything).Return(mockVersions, nil)

		// Create manager with airgap bundle
		store := appupgradestore.NewMemoryStore()
		manager, err := NewAppUpgradeManager(
			WithLogger(logger.NewDiscardLogger()),
			WithAppUpgradeStore(store),
