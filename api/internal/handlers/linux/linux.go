...
func New(cfg types.APIConfig, opts ...Option) (*Handler, error) {
	...
	if h.cfg.Mode == types.ModeInstall {
		...
		if h.installController == nil {
			...
			install.WithDisableFilesystemPerformanceCheck(h.cfg.DisableFilesystemPerformanceCheck),
		...
		if h.upgradeController == nil {
			...
			upgrade.WithDisableFilesystemPerformanceCheck(h.cfg.DisableFilesystemPerformanceCheck),
